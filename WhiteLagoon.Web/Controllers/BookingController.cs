...
namespace WhiteLagoon.Web.Controllers
{
    public class BookingController : Controller
    {
        ...
        public IActionResult GenerateInvoice(int id,string downloadType)
        {
            ...
            textSelection = document.Find("xx_payment_data", false, true);
            textRange = textSelection.GetAsOneRange();
            textRange.Text = bookingFromDb.PaymentDate.ToShortDateString();

            textSelection = document.Find("xx_checkin_data", false, true);
            textRange = textSelection.GetAsOneRange();
            textRange.Text = bookingFromDb.CheckInDate.ToShortDateString();

            textSelection = document.Find("xx_checkout_data", false, true);
            textRange = textSelection.GetAsOneRange();
            ...
            if (bookingFromDb.VillaNo > 0)
            {
                WTableRow row2 = table.Rows[2];

                row2.Cells[0].Width = 80;
                row2.Cells[1].AddParagraph().AppendText("Villa Number - " + bookingFromDb.VillaNo.ToString());
